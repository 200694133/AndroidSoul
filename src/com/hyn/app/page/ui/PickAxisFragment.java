 ...
 * <br/>
 * A complete axis line as value of one. The Leftmost pixel of the set value to zero and the
 * Rightmost of the line set the value to one. But it's not means that set the value in
...
public class PickAxisFragment extends Fragment{
    ...
    private int mLeftmostRawX = -1;
    private int mRightmostRawX = -1;
    ...
    public static PickAxisFragment instantiate(Context context, String fname, Bundle args) {
    	PickAxisFragment res = new PickAxisFragment();
    	if(null != args){
	    	res.mInternalLeftAxisPosition = args.getFloat(LEFT_POSITION_BUNDLE_KEY, DEFAULT_LEFT_POSITION);
	    	res.mInternalRightAxisPosition = args.getFloat(RIGHT_POSITION_BUNDLE_KEY, DEFAULT_RIGHT_POSITION);
	    	res.isDrawBackground = args.getBoolean(DRAW_BACKGROUND_BUNDLE_KEY, DEFAULT_DRAW_BACKGROUND);
    	}
    	return res;
    }
    ...
    public View onCreateView(LayoutInflater inflater, ViewGroup container, Bundle savedInstanceState) {
        ...
        mLeftPointView.setEnabled(false);
        mRightPointView.setEnabled(false);
        mViewGroup = view;
        mLeftIndicatorView.setVisibility(View.VISIBLE);
        mRightIndicatorView.setVisibility(View.VISIBLE);

        view.post(mInitialRunnable);

        return view;
    }

    private Runnable mInitialRunnable = new Runnable(){
		public void run() {
			 init();
             if(isDrawBackground){
                 drawDrawBackGround();
    ...
    /**
     * This function is very important, it's initialize all variable and set some limit.
     */
    private void init(){
        getParentRawX(true); //step 1, get parent left position
        getParentWidth(true); //step 2, get parent width
        getLeftmostRawX(true); //
        getRightmostRawX(true);
        setupPointPosition();
    ...
    /**
     * Setup position of the point view. It's be called when fragment be create at the first time or onConfigChanged.
     */
    final void setupPointPosition(){
        ...
        mLeftPointView.setTranslationX(left * width + getLeftmostRawX());
        mRightPointView.setTranslationX(right * width + getLeftmostRawX());
    ...
    /**
     * This function set the event delegate for touch mode, to extend touchable size for finger, to improve the user
     *  interaction. 
     * @param delegateView the view need to delegate
     * @param filter the filter if need to ignore the current event.
     * @param dragEvent a callback to listener touch/move event.
     */
     ...
     * */
    void onPositionMove(View pointView, TextView indicatorView, float newDrawPosition){
        indicatorView.setTranslationX(pointView.getTranslationX());
        newDrawPosition = newDrawPosition<0.00001F?0:newDrawPosition;
        newDrawPosition = newDrawPosition>0.99999F?1:newDrawPosition;
        float f  = getAcceleratedRatio(newDrawPosition);
        indicatorView.setText(getDescriptor(f));
        if(pointView == mLeftPointView){
            //update left point view
        	mDrawableLeftAxisPosition = newDrawPosition;
            mInternalLeftAxisPosition = f;
        }else if(pointView == mRightPointView){
            //update right point view
        	mDrawableRightAxisPosition = newDrawPosition;
            mInternalRightAxisPosition = f;
        }
    }

    void updateNewLeftPointViewPosition(float newTransitionX, boolean notify){
        float newX = newTransitionX;
        newX = newX > getRightmostRawX()?getRightmostRawX():newX;
        newX = newX < getLeftmostRawX()?getLeftmostRawX():newX;
        ...
        onPositionMove(mLeftPointView, mLeftIndicatorView, (newX-getLeftmostRawX()) / getVisibleWidth());
        if(notify){
            onPickedChange(mInternalLeftAxisPosition, mInternalRightAxisPosition);
        }
    }
     ...
     * @param notify if need to notify all listeners.
     */
    void updateNewRightPointViewPosition(float newTransitionX, boolean notify){
        ...
        onPositionMove(mRightPointView, mRightIndicatorView, (newX-getLeftmostRawX()) / getVisibleWidth());
        if (notify) {
            onPickedChange(mInternalLeftAxisPosition, mInternalRightAxisPosition);
        }
    }
    ...
    private int getVisibleWidth(boolean isForceRefresh){
        if(isForceRefresh || mVisibleWidth < 0){
            mVisibleWidth = getRightmostRawX(true) - getLeftmostRawX(true);
        }
    ...
    int getLeftmostRawX(){
        return getLeftmostRawX(false);
    }

    /**
     * Get the Leftmost position of both left point view and right point view.
     * @param isForceRefresh
     * @return
     */
    private int getLeftmostRawX(boolean isForceRefresh){
        if(isForceRefresh || mLeftmostRawX < 0){
        	float backLeftmost = mBackGroundView.getX() + getParentRawX();
//            mLeftmostRawX = getActivity().getResources().getDimensionPixelSize(R.dimen.pick_fragment_background_padding);
//            mLeftmostRawX -= getLeftPointViewSize()[0]/1.5F;
        	mLeftmostRawX = (int)(backLeftmost - getLeftPointViewWidth()/1.5F);
        }
        return mLeftmostRawX;
    }

    int getRightmostRawX(){
        return getRightmostRawX(false);
    }

    private int getRightmostRawX(boolean isForceRefresh){
        if(isForceRefresh || mRightmostRawX < 0){
//            mRightmostRawX = getParentWidth(true) - getActivity().getResources().getDimensionPixelSize(R.dimen.pick_fragment_background_padding);
//            mRightmostRawX -= 10;
        	int []l = new int[4];
        	mBackGroundView.getLocationInWindow(l);
        	float backRightmost = mBackGroundView.getWidth() +  l[0];
        	mRightmostRawX =  (int)(backRightmost - getRightPointViewWidth()/2);
        }
        return mRightmostRawX;
    }

    private int getLeftPointViewWidth(){
    	return mLeftPointView.getWidth();
    }
    
    private int getRightPointViewWidth(){
    	return mRightPointView.getWidth();
    }

    /**
     * Enable the point view.  User change interact with the fragment.
     */
    public void enable(){
    	if(null != mLeftPointView) mLeftPointView.setEnabled(true);
    	if(null != mRightPointView) mRightPointView.setEnabled(true);
    }

    public void enable(long delayMill){
    ...
    public void disable(){
    	if(null != mLeftPointView) mLeftPointView.setEnabled(false);
    	if(null != mRightPointView)  mRightPointView.setEnabled(false);
    }
    
    /**
     * Left and Right point view cannot touchable, and keep  mills mill seconds.
     * @param mills 
     */
    public void disable(long mills){
    	if(null != mLeftPointView) mLeftPointView.setEnabled(false);
    	if(null != mRightPointView)  mRightPointView.setEnabled(false);
    	enable(mills);
    }
    ...
    /**
     * Notify listeners that the new picked area.
